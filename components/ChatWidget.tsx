...
export default function ChatWidget({
  ...
  const ask = async () => {
    ...
    try {
      ...
      const sources = Array.from(
        new Set(
          (data.matches || [])
            .map((m: any) => m?.metadata?.source_url || m?.metadata?.source)
            .filter(Boolean)
        )
      );
      ...
      typeOut(`حدث خطأ: ${e?.message || e}`, [], null);
    } finally {
      setLoading(false);
    }
  };
  ...
  // Typewriter reveal
  const typeOut = (fullText: string, sources?: string[], logId?: number | null, cps = 32) => {
